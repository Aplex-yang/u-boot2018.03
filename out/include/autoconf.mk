...
CONFIG_SYS_LDSCRIPT="board/cmi_at151/u-boot.lds"
CONFIG_MII=y
...
CONFIG_BOARDDIR="board/cmi_at151"
...
CONFIG_EXTRA_ENV_SETTINGS="DEFAULT_LINUX_BOOT_ENV "boot_fdt=try0bootpart=0:20bootdir=/boot0bootfile=zImage0fdtfile=undefined0console=ttyO0,115200n80partitions=uuid_disk=${uuid_gpt_disk};name=rootfs,start=2MiB,size=-,uuid=${uuid_gpt_rootfs}0optargs=0mmcdev=00mmcroot=/dev/mmcblk0p2 ro0usbroot=/dev/sda2 ro0mmcrootfstype=ext4 rootwait0usbrootfstype=ext4 rootwait0rootpath=/export/rootfs0nfsopts=nolock0static_ip=${ipaddr}:${serverip}:${gatewayip}:${netmask}:${hostname}::off0ramroot=/dev/ram0 rw0ramrootfstype=ext20mmcargs=setenv bootargs console=${console} ${optargs} ${mtdparts} root=${mmcroot} rootfstype=${mmcrootfstype}0usbargs=setenv bootargs console=${console} ${optargs} ${mtdparts} root=${usbroot} rootfstype=${usbrootfstype}0spiroot=/dev/mtdblock4 rw0spirootfstype=jffs20spisrcaddr=0xe00000spiimgsize=0x3620000spibusno=00spiargs=setenv bootargs console=${console} ${optargs} root=${spiroot} rootfstype=${spirootfstype}0netargs=setenv bootargs console=${console} ${optargs} root=/dev/nfs nfsroot=${serverip}:${rootpath},${nfsopts} rw ip=dhcp0bootenv=uEnv.txt0loadbootenv=load mmc ${mmcdev} ${loadaddr} ${bootenv}0usbloadbootenv=load usb 0:1 ${loadaddr} ${bootenv}0importbootenv=echo Importing environment from mmc ...; env import -t $loadaddr $filesize0usbimportbootenv=echo Importing environment from USB ...; env import -t $loadaddr $filesize0ramargs=setenv bootargs console=${console} ${optargs} root=${ramroot} rootfstype=${ramrootfstype}0loadramdisk=load mmc ${mmcdev} ${rdaddr} ramdisk.gz0loadimage=load mmc ${bootpart} ${loadaddr} ${bootdir}/${bootfile}0usbloadimage=load usb 0:1 ${loadaddr} kernel-fit.itb0loadfdt=load mmc ${bootpart} ${fdtaddr} ${bootdir}/${fdtfile}0usbloados=run usbargs; bootm ${loadaddr}#conf${board_name}; if test $? -ne 0; then echo Using default FIT configuration; bootm ${loadaddr}; fi;0mmcloados=run mmcargs; if test ${boot_fdt} = yes || test ${boot_fdt} = try; then if run loadfdt; then bootz ${loadaddr} - ${fdtaddr}; else if test ${boot_fdt} = try; then bootz; else echo WARN: Cannot load the DT; fi; fi; else bootz; fi;0usbboot=usb reset; if usb storage; then echo USB drive found;if run usbloadbootenv; then echo Loaded environment from ${bootenv};run usbimportbootenv;fi;if test -n $uenvcmd; then echo Running uenvcmd ...;run uenvcmd;fi;if run usbloadimage; then run usbloados;fi;fi;0mmcboot=mmc dev ${mmcdev}; if mmc rescan; then echo SD/MMC found on device ${mmcdev};if run loadbootenv; then echo Loaded environment from ${bootenv};run importbootenv;fi;if test -n $uenvcmd; then echo Running uenvcmd ...;run uenvcmd;fi;if run loadimage; then run mmcloados;fi;fi;0spiboot=echo Booting from spi ...; run spiargs; sf probe ${spibusno}:0; sf read ${loadaddr} ${spisrcaddr} ${spiimgsize}; bootz ${loadaddr}0netboot=echo Booting from network ...; setenv autoload no; dhcp; tftp ${loadaddr} ${bootfile}; tftp ${fdtaddr} ${fdtfile}; run netargs; bootz ${loadaddr} - ${fdtaddr}0ramboot=echo Booting from ramdisk ...; run ramargs; bootz ${loadaddr} ${rdaddr} ${fdtaddr}0findfdt=setenv fdtfile am335x-baltos.dtb0" NANDARGS"
CONFIG_BOOTP_PXE=y
...
CONFIG_SPI=y
CONFIG_MTD_DEVICE=y
CONFIG_AM335X_USB0_MODE="MUSB_HOST"
...
CONFIG_AM335X_USB1_MODE="MUSB_OTG"
CONFIG_I2C=y
CONFIG_LIB_RAND=y
